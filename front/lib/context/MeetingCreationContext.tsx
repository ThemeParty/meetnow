...
const MeetingCreationContext = createContext<
  MeetingCreationContextType | undefined
>(undefined)

export const MeetingCreationProvider = ({
  children,
}: {
  children: ReactNode
}) => {
  ...
  return (
    <MeetingCreationContext.Provider
      value={{ meetingData, updateMeetingData, resetMeetingData }}
    >
      {children}
    </MeetingCreationContext.Provider>
  )
...
export const useMeetingCreation = () => {
  ...
  if (context === undefined) {
    throw new Error(
      'useMeetingCreation must be used within a MeetingCreationProvider',
    )
  }
  return context
}